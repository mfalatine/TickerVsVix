...
# Single-instance lock (prevents starting a second copy while one is running)
LOCKFILE_PATH = os.path.join(tempfile.gettempdir(), "spvsvix_app.lock")
SHUTDOWN_FLAG_PATH = os.path.join(tempfile.gettempdir(), "spvsvix_shutdown.flag")
LOCK_FD = None  # type: ignore[assignment]
SINGLE_INSTANCE_PORT = 51285
...
def request_shutdown_running_instance() -> None:
    try:
...
def start_shutdown_watcher(window_obj=None) -> None:
    def _watch():
        while True:
            try:
                if os.path.exists(SHUTDOWN_FLAG_PATH):
                    ...
                    try:
                        ...
                        pass
                    # Attempt graceful window destruction across pywebview backends
                    try:
                        import webview  # type: ignore
                        # Destroy default window; this should cause webview.start() to return
                        webview.destroy_window()  # type: ignore[call-arg]
                    except Exception:
                        try:
                            # Fallback: ask the OS to close our top-level window (Windows)
                            subprocess.Popen(["taskkill", "/PID", str(os.getpid()), "/T", "/F"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                        except Exception:
                            pass
                    # Proactively release lock and exit to ensure the second instance can start
                    try:
                        if LOCK_FD is not None:
                            try:
                                os.close(LOCK_FD)
...
def try_signal_shutdown_via_socket() -> bool:
    try:
        with closing(socket.create_connection(("127.0.0.1", SINGLE_INSTANCE_PORT), timeout=0.5)) as sock:
            sock.sendall(b"quit\n")
            return True
    except Exception:
        return False

def start_control_server_for_shutdown() -> None:
    def _server():
        try:
...
if __name__ == "__main__":
    # Enforce single instance: if another is running, ask it to shut down and wait a moment
    if not acquire_single_instance_lock():
        # Try socket signal first (preferred), then legacy flag
        sent = try_signal_shutdown_via_socket()
        if not sent:
            request_shutdown_running_instance()
        # Wait briefly for existing instance to close and release lock
        for _ in range(100):  # ~10s
            time_module.sleep(0.1)
            if not os.path.exists(LOCKFILE_PATH):
                break
        # Try to acquire again; if still not possible, exit quietly
        if not acquire_single_instance_lock():
            print("Another instance is closing. Please try again.")
            sys.exit(0)
    ...
    if use_ui:
        ...
        start_control_server_for_shutdown()
        start_shutdown_watcher(window)
        # Prefer modern Edge WebView2 on Windows so the UI's modern JS works
        try:
            webview.start(gui="edgechromium")
        except KeyboardInterrupt:
            # Graceful shutdown on Ctrl+C or backend termination
            pass
        except Exception:
            try:
                webview.start()
